  ...
  return (
    <React.Fragment>
      <ul className={`${style.fails__list} ${props.failAdd ? '' : ''}`}>
        {props.image.length === 0 && props.images.length === 0 ? null : (
          <ListItemImage
            handleDeleteFail={handleOpen}
            image={props.image}
            images={props.images}
          />
        )}
        {props.application.length === 0 &&
        props.applications.length === 0 ? null : (
          <ListItemDocument
            handleDeleteFail={handleOpen}
        ...
        {props.video.length === 0 && props.videos.length === 0 ? null : (
          <ListItemVideo
            handleDeleteFail={handleOpen}
            video={props.video}
            videos={props.videos}
        ...
        {props.audio.length === 0 && props.audios.length === 0 ? null : (
          <ListItemAudio
            handleDeleteFail={handleOpen}
            audio={props.audio}
            audios={props.audios}
